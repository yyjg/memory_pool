...
namespace Memory_Pool {
    ...
    void* ThreadCache::fetchFromCentralCache(size_t index) {
        ...
        size_t batchNum = getBatchNum(index);
    ...
    bool ThreadCache::shouldReturnToCentralCache(size_t index) {
        ...
        size_t threshold = 1024; // 例如，64个内存块
        return (freeListSize_[index] > threshold);
    }

    // 计算批量获取内存块的数量
    size_t ThreadCache::getBatchNum(size_t index) {
        // 批量获取不超过8KB大小
        ...
        // 根据对象大小设置合理的基准批量数
        size_t baseNum;
        if (index < 16) baseNum = 64;    // 64 * 64 = 4KB
        else if (index < 32) baseNum = 32;  // 32 * 128 = 4KB
        else if (index < 64) baseNum = 16; // 16 * 256 = 4KB
        else if (index < 128) baseNum = 8;  // 8 * 512 = 4KB
        else if (index < 256) baseNum = 4;  // 4 * 1024 = 4KB
        else if (index < 512) baseNum = 2; // 2 * 2048 = 4KB
        else baseNum = 1;                   // 大于1024的对象每次只从中心缓存取1个

        size_t maxNum = std::max(size_t(1), MAX_BATCH_SIZE / (index * ALIGNMENT));
